...
def main():
    # images
    img_folder = os.path.join('testing', 'shift_imgs_dial8_display')
    names_x = ['init','x1','x2','x3']
    names_y = ['init','y1','y2','y3']
    names_pol = ['0', '45', '90', '135']

    init = tifffile.imread(os.path.join(img_folder, 'initial.tif'))
    x1 = tifffile.imread(os.path.join(img_folder, 'x1.tif'))
    x2 = tifffile.imread(os.path.join(img_folder, 'x2.tif'))
    x3 = tifffile.imread(os.path.join(img_folder, 'x3.tif'))
    y1 = tifffile.imread(os.path.join(img_folder, 'y1.tif'))
    y2 = tifffile.imread(os.path.join(img_folder, 'y2.tif'))
    y3 = tifffile.imread(os.path.join(img_folder, 'y3.tif'))
    
    plt.imshow(init)
    plt.show()

    imgs_x = np.array([init, x1, x2, x3])
    imgs_y = np.array([init, y1, y2, y3])
